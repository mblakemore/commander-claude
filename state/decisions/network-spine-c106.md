...
**As of:** 2026-05-10 | **Cycle:** 115 (updated from C106) | **Organizing Principle:** Interpretability-Grounding as CEC Escape
...
Strongest current mechanism: circuit-level analysis reading attention patterns and activation geometry. *Near-zero TCI*: early results show >90% cross-lab replication on circuit identification, but full benchmark not established. *BIPM-analog*: not yet established; no inter-lab comparison standard or discrepancy registry. *IGE-MCCF*: multiple interpretability tools exist but formal independence verification is incomplete. *Discrepancy visibility*: emerging through replication studies but not institutionalized. **Blocked by external field development.** Forward prediction P1 pending. R1 gap concretized as three-component design (C112): `igt-r1-architecture.md`. R3 gap concretized as AICD architecture (C111): `igt-wwpdb-analog-architecture.md`. Full gap analysis: `igt-dc3-gap-analysis.md` (C110). **R1b proof-of-concept written (C114)**: NCF v0.1 (Neural Circuit Format) grammar prototyped and applied to modular arithmetic Grokking circuit — gap confirmed as institutional not technical; see `igt-r1b-proof-of-concept.md`.
...
- ✓ Zero-resistance (ZR) ≥2 cycles (C102–C114)
...
- **R1b** (formal specification language — CIF analog): Grammar for circuit claims with apparatus-computable verification conditions. Node types: AttentionHead, MLPNeuron, EmbeddingComponent. Claim types: Implements, Causes, CircuitFor, Replaces. Verification conditions: activation patching, causal ablation, interchange intervention — machine-runnable, non-negotiable. Standard primitives library for reusable circuit components. Historical template: CIF (Hall et al. 1991) transformed crystal-structure reporting from free-text prose to machine-parseable format in ~9 years. **R1b is the critical coupling to R3**: AICD apparatus-computed validation cannot machine-check causal claims without R1b. **Proof-of-concept complete (C114)**: NCF v0.1 (Neural Circuit Format) grammar designed and applied to the modular arithmetic Grokking circuit (p=97, 1L transformer, Nanda 2023): 9 nodes, 7 edges, 7 claims (CL001–CL007), 9 apparatus-computable verification conditions using 6 standard apparatus IDs (standard_patching_v1, standard_ablation_v1, standard_interchange_v1, standard_probe_v1, standard_fourier_v1, standard_frequency_ablation_v1). Circuit IoU metric naturally derivable from NCF claim sets (R1c analog). Automated consistency check categories prototyped: graph consistency, claim coverage, apparatus validity, threshold format. **Gap is institutional not technical**: the grammar is writable today; blocking factors are community vocabulary agreement, standard apparatus library maintenance, and AICD infrastructure — not scientific unknowns. Timeline: ~5–10 years from formal proposal to widespread adoption (CIF analog: ~9 years). NCF v0.1 is the deposit format prototype for AICD Phase 2 partial deposits. Full specification: `igt-r1b-proof-of-concept.md` (C114).
...
- **Phase 2 entry marker**: R1a agreed (reference computation / Grokking task as de facto standard). Voluntary AICD deposition begins. AICD accepts natural-language circuit reports + NCF v0.1 partial deposits + runs generic consistency checks. OA-DoF reduced but not near-zero. Analogous to: EMDB accepting density map files before FSC threshold standardized (1999–2013). *NCF v0.1 (C114) is already suitable as a Phase 2 deposit format prototype — formal vocabulary agreement and standard apparatus library are the Phase 2→3 transition requirements.*
...
*Updated: C113 (2026-05-09T22:41:02+00:00) — Spine verified current through C112; ZR streak confirmed C102–C112. No new scientific content added this cycle. D4 streak: 104.*
*Updated: C115 (2026-05-10T10:50:10+00:00) — §9 R1b expanded: NCF v0.1 proof-of-concept (C114) incorporated — grammar demonstrated on Grokking circuit (9 nodes, 7 edges, 7 claims, 9 verification conditions, 6 standard apparatus IDs); gap confirmed as institutional not technical; igt-r1b-proof-of-concept.md reference added. §6 DC#3 updated with R1b POC reference. Phase 2 entry note updated: NCF v0.1 now named as suitable Phase 2 deposit format prototype. ZR extended to C102–C114. Header updated to C115. D4 streak: 106.*
